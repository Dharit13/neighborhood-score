...
def cmd_status():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT table_name, record_count, status, last_seeded_at FROM data_freshness ORDER BY table_name"
            )
            rows = cur.fetchall()
            if not rows:
...
def main():
    ...
    sub_fetch.add_argument(
        "--slums", action="store_true", help="Download Bengaluru slum locations from data.opencity.in"
    )
    sub_fetch.add_argument(
        "--waste", action="store_true", help="Download BBMP waste infrastructure from data.opencity.in"
    )
    sub_fetch.add_argument(
        "--wards", action="store_true", help="Download GBA 369 ward boundaries and map to neighborhoods"
    )
    sub_fetch.add_argument(
        "--scrape-rera", action="store_true", help="Scrape K-RERA portal for all builder/project data"
    )
