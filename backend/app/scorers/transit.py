...
from app.models import NearbyDetail, TransitDetail, TransitScoreResult, score_label
from app.utils.geo import actual_walk_time, drive_time_estimate, haversine_km, marketing_walk_claim, walk_minutes
...
def _make_transit_detail(row: dict, transit_type: str, lat: float, lon: float) -> TransitDetail:
    ...
    return TransitDetail(
        ...
        walk_minutes=actual_min,
        actual_walk_km=actual_km,
        marketing_claim_minutes=marketing_min,
...
async def compute_transit_score(lat: float, lon: float) -> TransitScoreResult:
    ...
    final_score = round(
        min(max(0.35 * metro_score + 0.30 * bus_score + 0.20 * train_score + 0.15 * multi_modal, 0), 100), 1
    )
    ...
    if nearest_metro_detail:
        ...
        offpeak_travel = round(
            (
                nearest_metro_detail.walk_minutes
                if nearest_metro_detail.recommended_mode == "walk"
                else (nearest_metro_detail.drive_offpeak_minutes or nearest_metro_detail.walk_minutes)
            )
            + 3
            + 12,
            1,
    ...
    majestic_detail = _make_fixed_poi_detail(
        "Majestic Bus Station (Kempegowda)",
        "bus_station",
        lat,
        lon,
        MAJESTIC_BUS_LAT,
    ...
    return TransitScoreResult(
        ...
        nearest_metro=nearest_metro_detail,
        nearest_bus_stop=nearest_bus_detail,
        nearest_train=nearest_train_detail,
        airport=airport_detail,
        majestic=majestic_detail,
        city_railway=city_railway_detail,
        peak_travel_time_min=peak_travel,
        offpeak_travel_time_min=offpeak_travel,
        sources=SOURCES,
    )
